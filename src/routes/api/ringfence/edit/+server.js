...
export async function POST(event) {
	...
	try {
		...
		await supabase.from("wip").update(
			{
		...
		const partnerBody = `
			Dear Partner</p><p>Your Ringfence Request (Edit) has been sent to ${approverEmail} for approval.<br>
		...
		await sendEmail({
			...
			subject: `🟠 Ringfence Request (Edit) for ${itemData.title} (${itemId})`,
			body: partnerBody,
